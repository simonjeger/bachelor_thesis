    ...
    def picture_save(self):
        ...
        for x in range(len(self.my_robot)):
            ...
            for y in range(len(self.my_robot[x].position_robot_estimate)):
                ...
                if (contact == 1) & (x != y):
                    dx = self.my_robot[x].position_robot_estimate[y][0] - self.my_robot[x].position_robot_estimate[x][0]
                    dy = self.my_robot[x].position_robot_estimate[y][1] - self.my_robot[x].position_robot_estimate[x][1]
                    lin = patches.ConnectionPatch(self.my_robot[x].position_robot_estimate[x],self.my_robot[x].position_robot_estimate[y], "data", color=color_neighbour)
                    nei = patches.Circle((self.my_robot[x].position_robot_estimate[y]), radius=self.size_point, color=color_neighbour, fill=True)
                    ax[x, 0].add_patch(lin)
                    ax[x, 0].add_patch(nei)

            # Belief_state_position
            for y in range(len(self.my_robot[x].position_robot_estimate)):
                ax[x, y + 1].imshow(self.my_robot[x].my_belief_position.belief_state[y])
